...
def make_owner(modeladmin, request, querryset):
    for querry in querryset:
        user= querry 
        user.owner=True
        user.save()
        
make_owner.allowed_permissions=("add",)
make_owner.short_description = "make owner(s)"
...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('email', 'is_admin','is_staff','owner')
    list_filter = ('is_admin',)
    actions = [make_manager,make_owner]
    fieldsets = (
